...
class CosyVoice:
    ...
    def inference_zero_shot(self, tts_text, prompt_text, prompt_wav, zero_shot_spk_id='', stream=False, speed=1.0, text_frontend=True):
        # Only warn about <|endofprompt|> for CosyVoice3 when not using zero_shot_spk_id
        # (i.e., actual zero-shot mode where user provides reference audio)
        if (self.__class__.__name__ == 'CosyVoice3' and
            zero_shot_spk_id == '' and
            '<|endofprompt|>' not in prompt_text + tts_text):
